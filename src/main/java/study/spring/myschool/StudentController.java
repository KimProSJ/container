...
import study.spring.myschool.service.StudProfDeptService;
...
public class StudentController {
	...
	@Autowired
	StudProfDeptService studProfDeptService;
	...
	public ModelAndView StudList(Locale locale, Model model) {
		...
		int totalCount = 0;
		try {
			totalCount = studProfDeptService.getStudProfDeptCount(student);
		}  catch (Exception e) {
		...
		try {
			list = studProfDeptService.getStudProfDeptList(student);
		} catch (Exception e) {
			return web.redirect(null, e.getLocalizedMessage());
		}
	...
	public ModelAndView StudView(Locale locale, Model model) {
		...
		try {
			item = studProfDeptService.getStudProfDeptItem(student);
		} catch (Exception e) {
			return web.redirect(null, e.getLocalizedMessage());
		} 
	...
	public ModelAndView StudAddOk(Locale locale, Model model) {
		...
		if (tel == null) 		{ return web.redirect(null, "연락처를 입력하세요.");}
